...
export default function AuditLog() {
  ...
  useEffect(() => {
    getAuditLogs().then(data => setLogs(data)).catch(() => setLogs([]))
  }, [])
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-bg px-8 py-7">
      ...
      <p className="text-[13px] text-tx-3 mb-6">All user activity within Sekra AI</p>
      <div className="bg-card border border-line rounded-lg overflow-hidden">
        ...
        {logs.length === 0 ? (
          <div className="px-4 py-8 text-center text-[12px] text-tx-4">No audit logs yet.</div>
        ) : (
          logs.map((log, i) => (
